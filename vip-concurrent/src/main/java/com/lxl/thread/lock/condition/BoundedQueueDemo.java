 ...
 * 程，直到队列出现“空位”，
 * 
 * 通过add方法添加一个元素，通过remove()方法移出一个 元素。 以添加方法为例，首先需要获得锁，目的是确保数组修改的可见性和排他性。
...
public class BoundedQueueDemo {
	...
	public BoundedQueueDemo(int size) {
	...
	public void add(Object t) throws InterruptedException {
		...
		try {
			while (count == items.length)
				notFull.await();
			System.err.println("add位置" + addIndex + ",内容=" + t);
			Thread.sleep(100);
			items[addIndex] = t;
	...
	public Object remove() throws InterruptedException {
		...
		try {
			...
			System.out.println("remove位置" + removeIndex + ",内容=" + x);
			Thread.sleep(100);
	...
	public static void main(String[] args) throws InterruptedException {
		BoundedQueueDemo b = new BoundedQueueDemo(5);
		for (int i = 0; i < 10; i++) {
			b.add(i);
			if (i == 4) {
				b.initRemove();
